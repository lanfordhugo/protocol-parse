---
name: python-testing
description: AI Python 测试工作专家,基于 pytest 框架执行 AI 自验证回归测试。在以下场景使用: (1) AI 完成模块开发后编写测试验证功能正确性 (2) 基于 issue 或 PRD 文档进行需求验证 (3) 编写遵循 AAA 模式的 pytest 测试 (4) 使用 Mock/Fixture 隔离外部依赖 (5) 执行测试并检查覆盖率 (6) 根本原因分析并修复失败的测试 (7) 确保测试覆盖率达到 85% 以上
---
...
* **`python-coding-style.md`**: Python编码规范、命名约定、注释、类型提示
* **pytest官方文档**: pytest框架基础语法和最佳实践
* **unittest.mock文档**: Python标准库Mock使用指南

## 1.  核心约束与禁止项

> **以下约束条件必须严格遵守，违反将导致测试失效**
...
* **编码规范：** 遵循 `python-coding-style.md`，使用类型提示和文档字符串
...
** 重要：** 测试文件必须以 `test_` 开头，测试函数必须以 `test_` 开头，确保pytest能够自动发现。
...
* 提示参考`python-coding-style.md`和pytest文档
* 推荐使用参数化测试和测试标记
