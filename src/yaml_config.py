    ...
    def _validate_bitfield_groups(self):
        ...
        for group_data in self.groups:
            ...
            if group.start_bit + group.width > total_bits:
                raise ValueError(
                    f"Bitfield group '{group.name}': exceeds total bits ({total_bits})"
                )
...
class YamlConfigLoader:
    ...
    def _validate_fields(
        self, fields: List[Union[Field, Group]], config: ProtocolConfig, context: str
    ) -> List[str]:
        """验证字段列表"""
        errors = []
        field_ids = set()
        ...
        for field_item in fields:
            if isinstance(field_item, Field):
                ...
                if field_item.type not in config.types:
                    errors.append(
                        f"{context}: Unknown type '{field_item.type}' in field '{field_item.name}'"
                    )

                if field_item.enum and field_item.enum not in config.enums:
                    errors.append(
                        f"{context}: Unknown enum '{field_item.enum}' in field '{field_item.name}'"
                    )
                ...
                if field_item.repeat_by and field_item.repeat_by not in field_ids:
                    errors.append(
                        f"{context}: repeat_by references unknown field '{field_item.repeat_by}'"
                    )
